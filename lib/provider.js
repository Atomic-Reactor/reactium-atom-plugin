...
class Provider {
	constructor(suggestions, selector) {
        this.suggestions = suggestions;
		// offer suggestions only when editing plain text or HTML files
		this.selector = selector || '.text.plain, .text.html.basic, .source.js, .source.js, .source.json';
	}
	...
	findMatchingSuggestions(prefix) {
		...
		let prefixLower = prefix.toLowerCase().split('.').join('');

		let matchingSuggestions = this.suggestions.filter((suggestion) => {
			let textLower = suggestion.text.toLowerCase().split('.').join('');
